  ...
  <div class="bg-white rounded-lg shadow-md overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h3 class="text-lg font-semibold text-gray-900">Lista de Recursos Maestros</h3>
          <p class="text-sm text-gray-500">Total: {{ pagination.total }} recursos</p>
        </div>
    ...
    <div v-else-if="resources.length === 0" class="p-8 text-center">
      ...
      <h3 class="mt-2 text-sm font-medium text-gray-900">No se encontraron recursos</h3>
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center space-x-4 min-w-0">
            <div class="flex-shrink-0 w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
              <span class="font-semibold text-blue-800 text-xs">{{ resource.tipo.charAt(0) }}</span>
            </div>
            
            <div class="min-w-0">
              <div class="flex items-center space-x-2">
                <h4 class="font-medium text-gray-900 truncate">{{ resource.codigo }}</h4>
                <span class="px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                  {{ resource.unidad }}
                </span>
              </div>
              <p class="text-sm text-gray-600 truncate">{{ resource.nombre }}</p>
              <p class="text-xs text-gray-500 mt-1">
                {{ resource.tipo }} • 
                <span class="font-medium text-green-600">{{ formatCurrency(resource.precio_referencia) }}</span>
              </p>
            </div>
          </div>
          ...
          <div class="flex items-center space-x-3">
            <span :class="[
              ...
              {{ resource.activo ? 'Activo' : 'Inactivo' }}
            </span>
...
const pagination = computed(() => resourcesStore.pagination);
...
const applyFilters = async () => {
  ...
  await resourcesStore.fetchResources(filters);
};
...
const changePage = async (page) => {
  ...
  await resourcesStore.updateFilter('page', page);
};
...
watch(() => resourcesStore.filters, (newFilters) => {
  searchQuery.value = newFilters.search || '';
  selectedType.value = newFilters.tipo || '';
}, { deep: true });
</script>